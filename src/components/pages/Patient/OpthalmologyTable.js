  ...
  const fetchRecords = async (currentPage, id) => {
    ...
    try {
      const response = await get(
        `/Ophthalmology/list/patient/${id}/${currentPage}/10`
      );
      ...
      <div className="header-section">
        ...
        <div className="button-bar">
          <button
            className="btn-case"
            onClick={() =>
              navigate(`/doctor/patients/ophthalmology/${patientId}`)
            }
          >
            + Create a Case Note
          </button>
                    ...
                    <tr key={record.id}>
                      ...
                      <td>
                        <button
                          onClick={() => {
                            navigate(
                              `/doctor/patients/ophthalmology/${patientId}/?treatmentId=${record.id}`
                            );
                          }}
