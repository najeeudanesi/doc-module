...
const OphthalmologyForm = ({ patient }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await post("/Ophthalmology", payload);
      if (response.isSuccess) {
        navigate(
          `/doctor/patients/ophthalmology/${patientId}/?treatmentId=${response?.data?.ophthalmologyId}`
        );
  ...
  const getRecord = async () => {
    try {
      const response = await get(`/Ophthalmology/${treatmentId}`);
      if (response.isSuccess) {
  ...
  const fetcLabhData = async (treatmentId) => {
    ...
    try {
      const response = await get(
        `/patients/list/ophthalmology/${treatmentId}/1/10/lab-request`
      );
