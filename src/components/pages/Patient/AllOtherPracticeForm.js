...
const AllOtherPracticeForm = ({ patient }) => {
  const navigate = useNavigate();
  const [searchParams] = useSearchParams();
  const treatmentId = searchParams.get("treatmentId");
  const practiceLabel = searchParams.get("practice");
  ...
  const getRecord = async () => {
    try {
      const response = await get(`/SpecialistExaminations/${treatmentId}`);
      if (response) {
        console.log(response);
        setFormData({
          ...response,
          patientId: response?.patient?.id || 0,
 ...
 const handleSubmitSpecialist = async (e) => {
    ...
    try {
      ...
      console.log(response);
      toast.success("Specialist examination submitted successfully");
      navigate(0);
      return;
  ...
  const createTreatmet = async (load) => {
    let payload = {
      ...
      antenatalId: 0,
      specialistExaminationId: +treatmentId || 0,
      ...load,
    };
            ...
            <section className="">
              ...
              {treatmentId && (
                <MedicationTable
                  ...
                  data={{
                    treatmentType: "specialist",
                    treatmentId: treatmentId,
                  }}
                />
              )}
              {!treatmentId && (
                <button onClick={handleSubmitSpecialist} className="submit-btn">
                  Submit
                </button>
              )}
            </section>
...
export default AllOtherPracticeForm;
