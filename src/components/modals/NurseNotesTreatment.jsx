  ...
  const fetchData = async () => {
    try {
      const response = await get(
        `/patients/vital-by-patientId/?patientId=${patientId}&pageIndex=${1}&pageSize=${23}`
      );
      ...
      const res = await get(
        `/patients/${patientId}/nursenotes/${data?.visitId}`
      );
              ...
              <td>
                <div>
                  {data?.medications.map((medication, index) => (
                    <div className="text-start m-t-10" key={index}>
                      {index + 1 + ". "}
                      {medication.name}
                    </div>
                  ))}
                ...
                <div className="text-start flex gap-4 m-t-10">
                  ...
                  <p className="text-gray"> {data?.carePlan}</p>
                </div>
              </td>
            </tbody>
          </table>
