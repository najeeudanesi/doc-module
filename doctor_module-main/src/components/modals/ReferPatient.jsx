...
function ReferPatient({ closeModal, visit, vital, id, treatment }) {
    ...
    const fetchCategories = async () => {
        try {
            ...
            const data = response?.resultList.map((cat) => ({ value: cat?.id, label: cat?.itemName }));
    ...
    const addLabRequest = () => {
        if (selectedCategory && labCentre.trim() !== '') {
            ...
            setOtherTestRequests([...otherTestRequests, {
                labTest,
                labCentre
            }]);
        } else {
    ...
    const referPatient = async () => {
        ...
        console.log(payload);
        try {
            await post(`/patients/${id}/vital/${vital?.vitalId}/lab-request`, payload);
                ...
                <div className="p-40">
                    ...
                    <div className="m-t-20">
                        <div className='flex gap-8 flex-col'>
                            ...
                            <p className='text-sm m-t-20'>Dont see a category? Specify the name of the lab test</p>
                            <InputField
                                label="Lab Test"
                                name="labTest"
                                value={labTest}
                        ...
                        <div className='flex flex-h-end'>
                            <button className="btn m-t-10" onClick={addLabRequest}>+ Add</button>
                        </div>
                    </div>
                            ...
                            <table className="bordered-table-2 m-t-10">
                                ...
                                <tbody>
                                    {testRequests.map((request, index) => (
                                        <tr key={index}>
                                            <td>{index + 1}</td>
                                            <td>{categoryOptions.find(opt => opt.value === request.categoryItemId)?.label}</td>
